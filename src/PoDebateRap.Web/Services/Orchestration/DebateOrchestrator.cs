...
using PoDebateRap.Web.Factories;
using System;
using System.Text;
...
namespace PoDebateRap.Web.Services.Orchestration
{
    public class DebateOrchestrator : IDebateOrchestrator
    {
        ...
        private const int MaxDebateTurns = 6; // Total turns for the debate (3 rounds per rapper)
        ...
        private async Task GenerateTurnTextAsync(IAzureOpenAIService aiService, string currentRapper, string opponent, string role, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogError(ex, "Error generating turn {Turn} text for {Rapper}. Exception: {Message}", _currentState.CurrentTurn, currentRapper, ex.Message);
